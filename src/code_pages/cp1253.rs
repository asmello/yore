...
#[cfg(feature = "alloc")]
use crate::decoder::{
    complete::decode_helper as decode_helper_lossy, incomplete::decode_helper,
};
impl CP1253 {
    ...
    /// Decode a single CP1253 byte into its corresponding character,
    ...
    /// Encode a single Unicode `char` to its CP1253 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP1253.encode_char(c).unwrap()).collect();
