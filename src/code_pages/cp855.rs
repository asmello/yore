...
impl CP855 {
    ...
    /// Decode a single CP855 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP855 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP855.encode_char(c).unwrap()).collect();
