...
impl CP1258 {
    ...
    /// Decode a single CP1258 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP1258 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP1258.encode_char(c).unwrap()).collect();
