...
impl CP910 {
    ...
    /// Decode a single CP910 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP910 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP910.encode_char(c).unwrap()).collect();
