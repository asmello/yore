...
impl CP437 {
    ...
    /// Decode a single CP437 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP437 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP437.encode_char(c).unwrap()).collect();
