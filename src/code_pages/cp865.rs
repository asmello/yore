...
impl CP865 {
    ...
    /// Decode a single CP865 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP865 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP865.encode_char(c).unwrap()).collect();
