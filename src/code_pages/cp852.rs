...
impl CP852 {
    ...
    /// Decode a single CP852 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP852 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP852.encode_char(c).unwrap()).collect();
