...
pub struct CP860;
impl CP860 {
    /// Decode CP860 byte-encoding into UTF-8 string
    ///
    /// # Examples
    ///
    /// ```
    /// use yore::code_pages::CP860;
    ///
    ...
    /// Decode a single CP860 byte into its corresponding character.
    ///
    ...
    ///
    /// assert_eq!(CP860.encode("text").unwrap(), vec![116, 101, 120, 116]);
    ...
    /// Encode a single Unicode `char` to its CP860 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP860.encode_char(c).unwrap()).collect();
