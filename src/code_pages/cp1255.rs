...
impl CP1255 {
    ...
    /// Decode a single CP1255 byte into its corresponding character,
    ...
    /// Encode a single Unicode `char` to its CP1255 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP1255.encode_char(c).unwrap()).collect();
