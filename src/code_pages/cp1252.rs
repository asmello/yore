...
impl CP1252 {
    ...
    /// Decode a single CP1252 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP1252 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP1252.encode_char(c).unwrap()).collect();
