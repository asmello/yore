...
impl CP1257 {
    ...
    /// Decode a single CP1257 byte into its corresponding character,
    ...
    /// Encode a single Unicode `char` to its CP1257 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP1257.encode_char(c).unwrap()).collect();
