...
impl CP737 {
    ...
    /// Decode a single CP737 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP737 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP737.encode_char(c).unwrap()).collect();
