...
impl CP861 {
    ...
    /// Decode a single CP861 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP861 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP861.encode_char(c).unwrap()).collect();
