...
#[cfg(feature = "alloc")]
use crate::decoder::{
    complete::decode_helper_non_ascii as decode_helper_non_ascii_lossy,
    incomplete::decode_helper_non_ascii,
};
impl CP864 {
    ...
    pub fn decode_lossy(self, bytes: &[u8]) -> Cow<'_, str> {
        decode_helper_non_ascii_lossy(&DECODE_TABLE_LOSSY, bytes)
    }
    /// Decode CP864 byte-encoding into UTF-8 string
    ///
    ...
    /// Decode a single CP864 byte into its corresponding character,
    ...
    pub fn decode_byte(self, b: u8) -> Option<char> {
        ...
        let s = unsafe {
            core::str::from_utf8_unchecked(&entry.buf[..entry.len as usize])
        };
        s.chars().next()
    }
    ...
    /// Encode a single Unicode `char` to its CP864 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP864.encode_char(c).unwrap()).collect();
