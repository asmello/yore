...
impl CP1250 {
    ...
    /// Decode a single CP1250 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP1250 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP1250.encode_char(c).unwrap()).collect();
