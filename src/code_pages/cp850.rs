...
pub struct CP850;
impl CP850 {
    /// Decode CP850 byte-encoding into UTF-8 string
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Decode a single CP850 byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CP850 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP850.encode_char(c).unwrap()).collect();
