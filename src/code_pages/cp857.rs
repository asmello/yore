...
impl CP857 {
    ...
    /// Decode a single CP857 byte into its corresponding character,
    ...
    /// Encode a single Unicode `char` to its CP857 byte, or `None` if
    ...
    ///
    /// let s = "text";
    /// let bytes: Vec<u8> = s.chars().map(|c| CP857.encode_char(c).unwrap()).collect();
