...
fn main() -> Result<()> {
    ...
    // CP437G: CP437 + IBM-Graphics glyphs for the C0 control byte range,
    // per unicode.org's IBMGRAPH.TXT. All glyphs (including the ones at
    // bytes that also have ASCII control semantics: 0x09 ○, 0x0A ◙,
    // 0x0D ♪) are representable. Callers that care about treating `\n`
    // etc. as line-control should intercept the source `char` before
    // calling `encode_char` — yore's ASCII fast-path still encodes
    // `'\t'/'\n'/'\r'` to the same bytes as the glyphs.
    let cp437g = {
        let mut m =
            parsers::parse_unicode_dot_org("tables/unicode.org/CP437.txt")?;
        m[0x01] = Some('\u{263A}'); // ☺
        m[0x02] = Some('\u{263B}'); // ☻
        m[0x03] = Some('\u{2665}'); // ♥
        m[0x04] = Some('\u{2666}'); // ♦
