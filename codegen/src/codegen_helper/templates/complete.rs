...
use crate::{decoder::{self, CompleteEntry}, encoder::Encoder, CodePage};
...
impl CODERSTRUCT {
    ...
    /// Decode a single CODERSTRUCT byte into its corresponding character.
    ///
    ...
    /// Encode a single Unicode `char` to its CODERSTRUCT byte, or `None` if
